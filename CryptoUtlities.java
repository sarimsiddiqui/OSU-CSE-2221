...
public final class CryptoUtilities {
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        // if remainder is zero then it is even otherwise odd
        return check.isZero();
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        NaturalNumber one = new NaturalNumber2(1);
        NaturalNumber two = new NaturalNumber2(2);
        NaturalNumber original = new NaturalNumber2();
        original.copyFrom(n);

        // check if p is zero, if so return one
        if (p.isZero()) {
            n.transferFrom(one);

            // check if p is one, if so, calculate n%m
        } else if (p.compareTo(one) == 0) {
            n.transferFrom(n.divide(m));

            // p is greater than one
        } else {

            // check if p is even then recursively answer
            if (isEven(p)) {
                p.divide(two);
                powerMod(n, p, m);
                p.multiply(two);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        // run through first test
        powerMod(w, two, n);
        if (w.compareTo(one) == 0) {
            return true;
        }

        // run through second test
        w.copyFrom(copy);
        n.decrement();
        powerMod(w, two, n);
        if (w.compareTo(one) != 0) {
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            for (int i = 0; i < fifty; ++i) {
                ...
                // create random value and add one to fit inside requires clause
                NaturalNumber w = randomNumber(copy);
                w.increment();
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        // create checker and checks if n is prime
        boolean isPrime;
        isPrime = isPrime2(n);

        // if n is not prime, increase by odd numbers only
        while (!isPrime) {
            ...
            // check if n is even or odd and increase by odd number only
            NaturalNumber r = n.divide(two);
            if (r.isZero()) {
                n.copyFrom(copy);
                n.increment();
            } else {
